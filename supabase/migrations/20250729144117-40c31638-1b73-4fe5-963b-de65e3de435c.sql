-- Fix critical RLS policy vulnerability first (without constraints that might fail)
...
-- Create a more secure policy for public certificate verification
...
-- Add admin policy for viewing all certificates
CREATE POLICY "Admins can view all certificates" 
ON public.certificates 
FOR SELECT 
USING (public.is_admin(auth.uid()));

-- Add basic input validation constraints (without format check for now)
